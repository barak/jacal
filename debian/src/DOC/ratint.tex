...
%% pdftex ratint.tex
%%
%% (Plain TeX works too; substitute "tex" for "pdftex" above and run
%% the resulting ratint.dvi through dvips.)

%% This document was originally typeset with a private "macros.tex"
%% which is not part of the JACAL distribution. Everything it supplied
%% is supplied here instead. Eplain (Debian package
%% texlive-formats-extra) provides \today, \verbatim, \numberedfootnote,
%% \readtocfile, \eqdef, \eqref and -- by way of btxmac.tex -- the BibTeX
%% interface \cite, \bibliographystyle and \bibliography; the section
%% and table numbering is defined below.

\input eplain

%% US letter, as upstream's dvips/ps2pdf run produced.
\ifx\pdfpagewidth\undefined \else
  \pdfpagewidth=8.5in \pdfpageheight=11in
\fi

%% Eplain's \today would stamp the build date; this document is dated
%% by its release, not by when it happens to be typeset.
\def\today{30 March 2024}
...
\outer\def\section#1#2\par{%
  ...
  \message{#1}%
  \expandafter\writetocsection\expandafter{\the\sectionnumber}{#1}%
  \leftline{\bf\the\sectionnumber.\quad #1}%
  \nobreak\smallskip\noindent}
\def\writetocsection#1#2{\writetocentry{section}{#1. #2}}

\newcount\tablenumber
\def\tabdef#1{%
  \global\advance\tablenumber by 1
  \definexref{#1}{\the\tablenumber}{tab}%
  Table \the\tablenumber}
\def\tabref#1{Table \refn{#1}}
